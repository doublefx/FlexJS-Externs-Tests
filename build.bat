...
java -cp "%FALCON_CLASSPTH%" -Dflexlib="%FLEX_HOME%\frameworks" flex2.tools.Mxmlc -load-config=%FLEX_HOME%/frameworks/flex-config.xml -closure-lib=%CLOSURE_LIBRARY% -sdk-js-lib=%FLEX_HOME%/frameworks/js/FlexJS/libs -external-library-path=%FALCON_HOME%/externs/js/out/bin/js.swc -external-library-path=%FALCON_HOME%/externs/jquery/out/bin/jquery-1.9.swc -library-path=%PROJECT_HOME%/bin/HtmlContainer\HtmlContainer.swc -js-output-type=FLEXJS -output=%PROJECT_HOME%/bin/ExternsApp %PROJECT_HOME%/src/Main.as
